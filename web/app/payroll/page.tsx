...
function downloadPayslip(payslip: Payslip, periodName: string) {
  const html = `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8" />
  <title>Payslip – ${payslip.employee_name} – ${periodName}</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .company { font-size: 20px; font-weight: 800; color: #FF6600; }
    .employee { text-align: right; }
    .employee .name { font-size: 15px; font-weight: 700; }
    .employee .meta { font-size: 11px; color: #64748b; margin-top: 2px; }
    ...
    .net { display: flex; justify-content: space-between; align-items: center; margin-top: 24px; padding: 14px 16px; background: #d1fae5; border-radius: 10px; }
    .net .label { font-size: 14px; font-weight: 700; color: #065f46; }
    .net .value { font-size: 18px; font-weight: 800; color: #065f46; font-family: 'Courier New', monospace; }
...
<body>
  <div class="header">
    <div class="company">Page Group</div>
    <div class="employee">
      <div class="name">${payslip.employee_name}</div>
      <div class="meta">${payslip.position_title} · ${payslip.grade_name}</div>
      <div class="meta">${periodName}</div>
    </div>
  </div>

  <div class="section-title">Earnings</div>
  <div class="row"><span class="label">Gross Salary</span><span class="value">${fmt(payslip.gross_salary)}</span></div>
  <div class="row"><span class="label">Basic (70%)</span><span class="value">${fmt(payslip.basic_salary)}</span></div>
  <div class="row"><span class="label">Housing (15%)</span><span class="value">${fmt(payslip.housing_allowance)}</span></div>
  <div class="row"><span class="label">Transport (15%)</span><span class="value">${fmt(payslip.transport_allowance)}</span></div>

  <div class="section-title">Tax (PAYE)</div>
  <div class="row"><span class="label">Taxable Income</span><span class="value">${fmt(payslip.taxable_income)}</span></div>
  <div class="row"><span class="label">PAYE Tax</span><span class="value deduction">${fmt(payslip.paye_tax)}</span></div>

  <div class="section-title">Pension (PenCom)</div>
  <div class="row"><span class="label">Pensionable Earnings</span><span class="value">${fmt(payslip.pensionable_earnings)}</span></div>
  <div class="row"><span class="label">Employee Pension (8%)</span><span class="value deduction">${fmt(payslip.emp_pension)}</span></div>
  <div class="row"><span class="label">Employer Pension (10%)</span><span class="value employer">${fmt(payslip.employer_pension)}</span></div>

  <div class="net">
    <span class="label">Net Pay</span>
    <span class="value">${fmt(payslip.net_pay)}</span>
  </div>

  <div class="footer">Generated by PageOS · Page Group · Confidential</div>
</body>
</html>`;
    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: "var(--pg-card)", border: "1px solid var(--pg-card-border)" }}>
      <div className="px-5 py-4" style={{ borderBottom: "1px solid var(--pg-row-border)", background: "linear-gradient(135deg,#FF660018,#7c3aed12)" }}>
        <div className="flex items-start justify-between">
          ...
          <button
            onClick={() => downloadPayslip(payslip, periodName)}
            className="flex items-center gap-1.5 h-8 px-3 rounded-lg text-[12px] font-semibold text-white shrink-0"
